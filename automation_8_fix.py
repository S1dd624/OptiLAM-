...
ANY_CLICK_WAIT_TIME = (
    0.1  # Time to wait for any random click's task to be execuited by EzCad
)
FUNCTION_INTERVAL_WAIT_TIME = (
    0.2  # Time to wait after performing a function in print3dItem()
)
RETRY_WAIT_TIME = (
    0.2  # Time to wait while retrying execuition of a function in print3dItem()
)
...
def selectFirstObjectInList(EzCadAppRef):
    ...
    EzCadAppRef["Toolbar3"].button(1).click()

    # Reselect the "Pick" Tool
    EzCadAppRef["Toolbar3"].button(0).click()

    # Click the title of "Object List"
    objectHeader = EzCadAppRef["Header"]
    objectHeader.click()
...
def showRedBoundary(app, EzCadAppRef):
    ...
    EzCadAppRef["Red(F1)"].click()

    while True:
        if app[
            "Mark"
        ].exists():  # If the Red boundary window is open, wait 1 second and recheck if it's open, if yes then wait again else return from the function
            time.sleep(1)
        else:
            break
...
def setXtoZero(EzCadAppRef):
    ...
    EzCadAppRef["Edit2"].type_keys("{END}+{HOME}-1")
    EzCadAppRef["&Apply"].click_input()
...
def hatchObject(app, EzCadAppRef, is_programs_first_layer):
    ...
    EzCadAppRef.menu_item("&Edit->Hatch\\tCtrl+H").click()

    time.sleep(ANY_CLICK_WAIT_TIME)
    if (
        is_programs_first_layer == 1
    ):  # Only wait for user input in setting the hatching property for the 1st loop item.
        while True:
            if (
                app.Hatch.exists()
            ):  # If the Hatching property window is open, wait 1 second and recheck if it's open, if yes then wait again else return from the function
                time.sleep(1)
            else:
                break
    else:
        app.Hatch["&OK"].click_input()
...
def selectMarkingProperty(EzCadAppRef, iter, is_programs_first_layer):
    ...
    if iter == 0:  # Black color parameters
        ...
        EzCadAppRef["Button12"].click_input()

        if is_programs_first_layer == 1:

            # Uncheck default param
            if EzCadAppRef["Use default param"].get_check_state():
                EzCadAppRef["Use default param"].click()

            # Set Speed(MM/Second) for black parameter
            EzCadAppRef["Spin1"].click()
            speed = 750
            send_keys(f"{speed}")

            # Set Power% for black parameter
            EzCadAppRef["Spin2"].click()
            power = 50
            send_keys(f"{power}")

            # Click Apply button
            EzCadAppRef["&Apply2"].click()

    elif iter == 1:

        # Click Blue color Button
        EzCadAppRef["Button13"].click_input()
...
def startMarking(app, EzCadAppRef, marking_time):
    ...
    EzCadAppRef["Mark(F2)"].click_input()
    # time.sleep(marking_time)   # Sleep till the printing is completed

    while True:
        if app[
            "Mark"
        ].exists():  # If the Marking property window is open, wait 1 second and recheck if it's open, if yes then wait again else return from the function
            time.sleep(1)
        else:
            break
...
def print3dItem(
    app,
    EzCadAppRef,
    numberOfObjectsInSVG,
    begin_at_layer_no,
    const_printing_interval,
    marking_time,
):
    """
    ...
    if current_loop_count == 0 and begin_at_layer_no > 0:
        ...
        current_loop_count += (
            begin_at_layer_no - 1
        )  # If begin_at_layer_no = 3 then current_loop_count must be = 2
    ...
    while (current_loop_count < numberOfObjectsInSVG) and (should_pause != True):
        ...
        if ((begin_at_layer_no <= 0) and (current_loop_count > 1)) or (
            (begin_at_layer_no > 0) and (current_loop_count == begin_at_layer_no)
        ):
            is_programs_first_layer = 0
        ...
        try:
            ...
            clickEnableInHatching(
                EzCadAppRef, 1
            )  # Firstly, we enable the hatching for black one
        ...
        try:
            ...
            clickEnableInHatching(
                EzCadAppRef, 0
            )  # Here we disable the hatching for the blue one
        ...
        if (
            should_pause != True
        ):  # To check if pause has been pressed while printing, if 'yes', then don't sleep for new layer printing
            # Constant time that this function will pause between printing of each layer
            print(
                f"\t- Waiting for 'Next Layer Delay: {const_printing_interval} sec' time"
            )
...
def waitForResizing(resize_wait_time):
    ...
    label = tkinter.Label(
        parent, text=f"Remaining Wait Time: {resize_wait_time} seconds"
    )
    label.pack(fill="x")

    resizing_wait_time_label = tkinter.Label(
        parent, text='Manually select "View Workspace" button'
    )
...
def resume_process(
    ...
    time.sleep(
        0.1
    )  # Just for Safety purpose as parallel execution of process may give rise to errors
    if is_paused:
        is_paused = False
...
def update_current_layer_count(layer_info, numberOfObjectsInSVG, gui_controls_root):
    ...
    layer_info.config(
        text=f"Printing layer: {current_loop_count + 1} of {numberOfObjectsInSVG}"
    )

    if should_pause == False:
        gui_controls_root.after(
            1000,
            lambda: update_current_layer_count(
                layer_info, numberOfObjectsInSVG, gui_controls_root
            ),
        )


def createControlGUI(
    app,
    EzCadAppRef,
    numberOfObjectsInSVG,
    begin_at_layer_no,
    ...
    pause_button = tkinter.Button(
        gui_control_frame,
        text="Pause",
        command=pause_process,
        width=16,
        relief="groove",
    )
    pause_button.grid(row=1, column=1, padx=10, pady=10)

    resume_button = tkinter.Button(
        gui_control_frame,
        text="Resume",
        command=lambda: resume_process(
            app,
            EzCadAppRef,
            numberOfObjectsInSVG,
            begin_at_layer_no,
    ...
    layer_info = tkinter.Label(
        layer_info_frame, text=f"Printing layer: 1 of {numberOfObjectsInSVG}"
    )
...
def begin(resizingWaitTime, begin_at_layer_no, const_printing_interval, marking_time):
    ...
    app = Application(backend="win32").start(
        cmd_line='"C:\\Users\\rmlab#1\\Desktop\\EZCAD3 driver\\EZCAD2 For AiO (20220915 Release)\\EZCAD2 For AiO.exe" '
    )
    EzCadAppRef = app["EzCad2.14.11 - No title"]
    ...
    EzCadAppRef.menu_item("&File->Import Vector File...\\tCtrl+B").click()
    ...
    EzCadAppRef["&Apply"].click()

    # Ungroup elements
    EzCadAppRef.menu_item("&Edit->UnGroup\\tCtrl+U").click()
...
class GUI:
    def __init__(self, root):
        ...
        self.file_entry = tkinter.Entry(
            self.select_file_section,
            textvariable=self.file_location,
            width=50,
            relief="groove",
        )
        ...
        self.const_printing_interval_label = tkinter.Label(
            self.select_file_section, text="Next Layer Delay"
        )
        self.const_printing_interval_label.grid(row=1, column=0, padx=10, pady=10)

        self.const_printing_interval_entry = tkinter.Entry(
            self.select_file_section,
            textvariable=self.const_printing_interval,
            width=15,
            relief="groove",
        )
        ...
        self.resizing_wait_time_label = tkinter.Label(
            self.select_file_section, text="Window Resizing time"
        )
        self.resizing_wait_time_label.grid(row=2, column=0, padx=10, pady=10)

        self.resizing_wait_time_entry = tkinter.Entry(
            self.select_file_section,
            textvariable=self.resizing_wait_time,
            width=15,
            relief="groove",
        )
        ...
        self.marking_time_label = tkinter.Label(
            self.select_file_section,
            text="Lasor movement time for hatching (not in use)",
        )
        self.marking_time_label.grid(row=3, column=0, padx=10, pady=10)

        self.marking_time_entry = tkinter.Entry(
            self.select_file_section,
            textvariable=self.marking_time,
            width=15,
            relief="groove",
        )
        ...
        self.calculate_layers_section = tkinter.LabelFrame(
            self.root, text="Click to calculate number of layers"
        )

        # Calculate the total number of layers in imported file
        self.start_button = tkinter.Button(
            self.calculate_layers_section,
            text="Calculate number of Layers",
            command=self.calc_no_of_layers,
            width=30,
            relief="groove",
        )
        ...
        calculated_no_of_layers_label = tkinter.Label(
            self.calculate_layers_section, text=f"Ans: {self.no_of_layers.get()} layers"
        )
        ...
        self.begin_at_layer_no_label = tkinter.Label(
            self.calculate_layers_section, text="Begin at Layer number: "
        )
        self.begin_at_layer_no_label.grid(row=1, column=0, padx=10, pady=10)

        self.begin_at_layer_no_entry = tkinter.Entry(
            self.calculate_layers_section,
            textvariable=self.begin_at_layer_no,
            width=15,
            relief="groove",
        )
    ...
    def browse_file(self):
        ...
        file_path = filedialog.askopenfilename(
            title="Select a File",
            filetypes=(
                ("All Vector files", "*.ai *.plt *.dxf *.dst *.svg *.nc *.g *.gbr"),
                ("all files", "*.*"),
            ),
        )
    ...
    def calc_no_of_layers(self):
        ...
        if self.file_location.get() != "":
            ...
            global calculated_no_of_layers_label
            calculated_no_of_layers_label.config(
                text=f"{self.no_of_layers.get()} layers"
            )
    ...
    def start_process(self):
        ...
        ):  # If file location is selected, only then should the program start
            gui_root.destroy()
            if self.no_of_layers.get() <= 0:
                self.calc_no_of_layers()
            begin(
                self.resizing_wait_time.get(),
                self.begin_at_layer_no.get(),
                self.const_printing_interval.get(),
                self.marking_time.get(),
            )
